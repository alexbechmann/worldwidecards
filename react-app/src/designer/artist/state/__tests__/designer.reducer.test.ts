import { artistDesignerReducer } from '../artist-designer.reducer';
import { addTextShape, updateShapePosition, toggleAllowUserEdit } from '../artist-designer.actions';
...
it('should toggle user edit', () => {
  ...
  var state = artistDesignerReducer(defaultState, action);
  expect(state.activeCard!.pages[0].shapes[1].allowUserEdit).toEqual(false);

  state = artistDesignerReducer(state, action);
